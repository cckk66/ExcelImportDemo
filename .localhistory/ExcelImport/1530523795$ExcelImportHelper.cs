    ...
    public class ExcelImportHelper
    {
        ...
        public virtual List<T> DataTableToList<T>(string filePath, string sheetName) where T : IExeclImport, new()
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                {
                    string ExeclMappingPropertyTimeKey = $"{TName}_{Prty.Name}_ExeclMappingPropertyTimeKey";
                    ExeclMappingAttribute PrtyExeclMapping = CacheHelper.GetCacheExeclMappingAttribute > (ExeclMappingPropertyTimeKey);
