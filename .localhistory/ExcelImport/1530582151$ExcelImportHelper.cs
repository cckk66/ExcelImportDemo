...
namespace ExcelImport
{
    public class ExcelImportHelper
    {
        ...
        public virtual List<T> DataTableToList<T>(string filePath, string sheetName) where T : IExeclImport, new()
        {
            ...
            {
                PropertyInfos = TType.GetProperties().Where(m => m.GetCustomAttributes(typeof(ExeclMappingAttribute), true) != null);
                if (PropertyInfos != null)
                ...
                foreach (PropertyInfo Prty in PropertyInfos)
                {
                    ...
                    if (dt.Columns.Contains(PrtyExeclMapping.ExeclHeadName))
                    {
                        ...
                        if (value != DBNull.Value && value != null && !string.IsNullOrEmpty(value.ToString()))
                        {
                            ...
                            IEnumerable<BaseValidationAttribute> baseValidationAttributeArray = CacheHelper.GetCache<IEnumerable<BaseValidationAttribute>>(BaseValidationPropertyKey);
                            if (baseValidationAttributeArray == null && !CacheHelper.IsExist(ConvertValuePropertyKeyNoHave))
                            {
                                baseValidationAttributeArray = Prty.GetCustomAttributes<BaseValidationAttribute>(true);
                            ...
                            if (baseValidationAttributeArray != null)
                            {
                                foreach (BaseValidationAttribute baseValidationAttribute in baseValidationAttributeArray)
                                {
                                    if (!baseValidationAttribute.IsValid(value))
                                    {
                                        PropertyInfo ExeclRowErrorMsgProperty = PropertyInfos.Where
