            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                foreach (PropertyInfo Prty in PropertyInfos)
                {
                    ...
                    if (dt.Columns.Contains(PrtyExeclMapping.ExeclHeadName))
                    {
                        ...
                        //如果非空，则赋给对象的属性  
                        if (value != DBNull.Value && value != null && !string.IsNullOrEmpty(value))
                        {

                            AppointValueAttribute appointValueAttribute = Prty.GetCustomAttribute<AppointValueAttribute>(true);
                            if (appointValueAttribute != null)
                            {
                                value = appointValueAttribute.GetAppointValue(value);
                            }
