                ...
                foreach (PropertyInfo Prty in PropertyInfos)
                {
                    ...
                    if (PrtyExeclMapping != null && dt.Columns.Contains(PrtyExeclMapping.ExeclHeadName))
                    {
                        ...
                        if (value != DBNull.Value && value != null && !string.IsNullOrEmpty(value.ToString()))
                        {
                            ...
                            //if (Prty.Name == "Age")
                            //{
                            //    var  excelBaseValidationAttribute1 = Prty.GetCustomAttributes<ExcelBaseValidationAttribute>(true);
                            //    ExcelBaseValidationAttribute excelBaseValidationAttribute = Prty.GetCustomAttribute<ExcelBaseValidationAttribute>(true);
                                    ...
                                    if (!baseValidationAttribute.IsValid(value))
                                    {
                                        ...
                                        if (ExeclRowErrorMsgProperty != null)
                                        {
                                            ExeclRowErrorMsgProperty.SetValue(t,
                                               string.IsNullOrEmpty(baseValidationAttribute.ErrorMsg) ? $"{baseValidationAttribute.GetType().Name} 验证未通过" : baseValidationAttribute.ErrorMsg
                                               , null);
                                            isValid = false;
                                            continue;
                                        }
